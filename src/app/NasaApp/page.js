...
export default function page() {
  ...
  prev2Date.setDate(prev2Date.getDate() - 1)
  const formattedPrev2Date = prev2Date.toJSON().slice(0, 10);
  ...
  const getAPODIMG = async () => {
    try{
      const APOD_API = `https://api.nasa.gov/planetary/apod?api_key=${process.env.NEXT_PUBLIC_NASA_API_KEY}`
  ...
  const getMARSIMG = async () => {
    try{   
      ...
      if(MARS_data.photos.length === 0){
        const MARS_API = `https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?earth_date=${formattedPrev2Date}&api_key=${process.env.NEXT_PUBLIC_API_KEY}`
      
        MARS_response = await fetch(MARS_API)
        MARS_data = await MARS_response.json()
        console.log(formattedPrev2Date)
    ...
    <div className="min-h-screen bg-linear-to-b from-indigo-900 to-purple-900 font-serif p-5 flex flex-row justify-center text-white">
      <div className="h-full w-[800px] text-center">
        ...
        <div className="grid grid-cols-2 grid-rows-4 gap-4">
          <div className="max-h-fit bg-blue-100 row-span-2 flex flex-col items-center p-4 rounded-3xl bg-radial from-blue-950 to-sky-700 shadow-md shadow-cyan-500/50 hover:shadow-inner cursor-pointer" onClick={() => router.push(`/NasaApp/apod?start=${currentDate}&end=${currentDate}`)}>
          ...
          <div className="bg-blue-100 col-start-2 row-start-2 row-span-2 flex flex-col items-center p-4 rounded-3xl bg-radial from-blue-950 to-sky-700 shadow-md shadow-cyan-500/50">
            ...
            {/* {MARS_IMG && <img className="h-48" src={MARS_IMG.photos[0].img_src}></img>} */}
