...
export default function ApodItem({ item }) {
  return (
    <Link className="cursor-default" href={`apod/${item.date}`}>
      <div className="h-80 flex flex-col items-center text-center font-serif text-white bg-linear-to-b from-indigo-500 to-emerald-800 m-5 p-5 rounded-lg shadow-md shadow-cyan-200 hover:shadow-inner">
        <p>{item.date}</p>
